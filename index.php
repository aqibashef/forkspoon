<?php get_header(); ?>
	...
	<div class="container">
		...
		<div class="row">
			<div class="<?php
							if(get_theme_mod('fs_sidebar_homepage') == true) { 
								echo 'col-sm-12 fullwidth';
							} else {
								echo 'col-sm-8'; 
							} 
							if(get_query_var('paged') == 0 && get_theme_mod('fs_home_layout') == 'full_grid') echo ' full_grid';
						?>">
				
					<?php $index = 0; //counting posts ?>

					<?php if(get_theme_mod('fs_home_layout') == 'grid' || get_theme_mod('fs_home_layout') == 'full_grid') : ?><ul class="row sp-grid"><?php endif; ?>
					
					<?php if (have_posts()) : while (have_posts()) : the_post(); $index++; ?>
						
						<?php if(get_theme_mod('fs_home_layout') == 'grid') : ?>
					...
					<?php if(get_theme_mod('fs_home_layout') == 'grid' || get_theme_mod('fs_home_layout') == 'full_grid') : ?></ul><?php endif; ?>
			...
			<?php if(get_theme_mod('fs_sidebar_homepage')) : else : ?>
